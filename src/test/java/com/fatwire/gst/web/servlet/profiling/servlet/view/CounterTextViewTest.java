...
public class CounterTextViewTest {

    @Test
    public void testRenderDocumentOff() throws IOException {
        ...
        final CounterTextView v = new CounterTextView(root, false, 1, counterHistory, timeHistory);
        final StringWriter s = new StringWriter();
        final PrintWriter pw = new PrintWriter(s);
        final PlainTextFormat format = new PlainTextFormat(pw);
        v.renderDocument(format);
        System.out.println(s.toString());
    }

    @Test
    public void testRenderDocumentOn() throws IOException {
        final CounterHistory counterHistory = new CounterHistory(15 * 60);
        final CounterHistoryBigInteger timeHistory = new CounterHistoryBigInteger(15 * 60);

        final ResponseTimeStatistic root = new ResponseTimeStatistic("/cs");
        final CounterTextView v = new CounterTextView(root, true, 1, counterHistory, timeHistory);
        final StringWriter s = new StringWriter();
        final PrintWriter pw = new PrintWriter(s);
        final PlainTextFormat format = new PlainTextFormat(pw);
        v.renderDocument(format);
